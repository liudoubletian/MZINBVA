...
data.pro <- function(kk,nodes=3){
  ...
  dd <- sandwich_cov(beta,data=data.split,phi,gamma,lamdab,lamdah,lamdad,lamdaf,psib=psib.split,psig=psig.split,pijk=pijk.split,verbose=FALSE,nodes=nodes,offset=off.split)
...
total_elbo_split <- function(beta,data,gamma,phi,psib,psig,lamdab,lamdah,lamdad,lamdaf,pijk,nodes=nodes,offset){
...
one_elbo_theta=function(theta,data,gamma,lamdad,lamdaf,psib,psig,pijk,nodes=nodes,offset){
  Y <- data$y;ID <-data$ID;cluster <-data$cluster;x1 <-data$x1;x2 <-data$x2
  p1 <- ncol(x1);
...
one_elbo_thetapsi=function(thetapsi,psig,data,pijk,gamma,lamdad,lamdaf,nodes=nodes,offset){
  Y <- data$y;ID <-data$ID;cluster <-data$cluster;x1 <-data$x1;x2 <-data$x2
  p1 <- ncol(x1);
...
sandwich_cov <- function(beta,data,phi,gamma,lamdab,lamdah,lamdad,lamdaf,psib,psig,pijk,verbose=FALSE,nodes=nodes,offset) {
  library(parallel)
  library(MASS)
  library(numDeriv)
  N <- length(data)
